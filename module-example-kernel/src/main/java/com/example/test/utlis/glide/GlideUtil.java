package com.example.test.utlis.glide;
...
import com.example.test.app.BaseApp;
import com.example.test.utlis.glide.transform.BlurTransformation;
import com.example.test.utlis.glide.transform.MatrixTransformation;
...
public abstract class GlideUtil   {
    ...
    public static void preload(String url, @NonNull final Map<String, String> headerMap, RequestListener<File> listener) {
        RequestBuilder<File> builder = Glide.with(getContext()).downloadOnly().load(getModel(url, headerMap));
...
//            corners = Math.max(corners, getContext().getResources().getDimensionPixelOffset(R.dimen.px6dp));
        ...
        RequestOptions requestOptions = getDefaultRequestOptions()
            .bitmapTransform(new RoundedCorners(corners));
        Glide.with(getContext()).load(mipmap)
            .apply(requestOptions)
            .into(imageView);
    }
